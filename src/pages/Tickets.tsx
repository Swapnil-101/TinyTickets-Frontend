    ...
    const loadTickets = async () => {
      try {
        ...
        for (const project of projectsData) {
          try {
            const projectMembers = await memberService.getProjectMembers(project._id);
            allMembers.push(...projectMembers);
          } catch (err) {
            console.error(`Failed to load members for project ${project._id}:`, err);
          }
        }
        ...
        for (const project of projectsData) {
          try {
            const projectTickets = await ticketService.getProjectTickets(project._id);
            allTickets.push(...projectTickets);
          } catch (err) {
            console.error(`Failed to load tickets for project ${project._id}:`, err);
          }
        }
    ...
    <div className="space-y-4 sm:space-y-6">
      <div className="bg-white shadow rounded-lg p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 space-y-4 sm:space-y-0">
          ...
          <Link
            to="/projects"
            className="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-lg font-medium text-center w-full sm:w-auto"
          >
            View Projects
          </Link>
        </div>
                    ...
                    <div className="flex flex-col space-y-3">
                      <div className="flex-1">
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center space-y-2 sm:space-y-0 sm:space-x-4 text-sm text-gray-500">
                          ...
                          <span>Project: {projects.find(p => p._id === ticket.projectId)?.name || 'Unknown'}</span>
                        </div>
                      </div>
